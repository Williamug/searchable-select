...
<div x-data="{
    ...
    <div @click="toggleDropdown()"
        ...
        <div class="flex items-center gap-2 px-3 py-2 min-h-[42px]">
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                {{-- Clear button --}}
                <span x-show="clearable && selectedValues.length > 0 && !disabled"
                    x-cloak
                    @click.stop="clearAll()"
    ...
    <div x-show="open && !disabled"
        ...
        <input type="text"
            x-ref="searchInput"
            x-model="search"
            @input.debounce.300ms="searchApi()"
            @click.stop
        ...
        <div class="max-h-60 overflow-auto overscroll-contain" x-ref="optionsList">
            ...
            @if ($grouped)
                <template x-for="group in filteredOptions" :key="group.group">
                    <div>
                        <div class="px-3 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-zinc-900 uppercase tracking-wider" x-text="group.group"></div>
                ...
                <div x-show="filteredOptions.length === 0 && !loading" class="px-3 py-3 text-gray-500 dark:text-gray-400 text-sm text-center">
                    {{ $emptyMessage }}
                </div>
            @else
