...
@php
    $selectedValues =
        $multiple && is_array($selectedValue) ? $selectedValue : ($selectedValue ? [$selectedValue] : []);
...
})" @click.outside="closeDropdown()" @keydown="handleKeydown" class="relative" wire:ignore.self>

    {{-- Trigger --}}
    <div @click="toggleDropdown()"
        {{ $attributes->except(['options', 'wireModel', 'placeholder', 'searchPlaceholder', 'disabled', 'emptyMessage', 'selectedValue', 'optionValue', 'optionLabel', 'multiple', 'clearable', 'apiUrl', 'apiSearchParam', 'grouped', 'groupLabel', 'groupOptions'])->merge(['class' => 'w-full text-left border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-zinc-800 text-gray-900 dark:text-white cursor-pointer select-none transition-shadow']) }}
        :class="{
    ...
    <div x-show="open && !disabled" x-cloak x-transition:enter="transition ease-out duration-100"
        ...
        <div class="max-h-60 overflow-auto overscroll-contain" x-ref="optionsList">
            ...
            <div x-show="error && !loading"
                class="px-3 py-3 text-red-500 dark:text-red-400 text-sm text-center">
                        ...
                        <template x-for="option in group.items" :key="option.value">
                            <div @click="toggleSelection(option.value)"
                                ...
                                :class="{
                                    ...
                                    'bg-gray-100 dark:bg-gray-700': flatOptions[highlightedIndex]?.value === option.value
                                }"
